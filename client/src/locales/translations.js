export const translations = {
  EN: {
    // Navigation
    nav: {
      aboutUs: "About Us",
      travelRequirements: "Travel requirements",
      myApplications: "My applications",
      signIn: "Sign In",
      signUp: "Sign Up",
      profile: "Profile",
      history: "History",
      signOut: "Sign Out",
      menu: "Menu",
      close: "Close"
    },

    // Language
    language: {
      english: "English",
      spanish: "Español"
    },

    // Hero Section
    hero: {
      title: "Fast, Simple, And Secure Visa Processing With Live Support.",
      subtitle: "Your One-Stop Solution for Mexican Passport Visa Applications!",
      country: "Country",
      startApplication: "Start Your Application"
    },

    // Countries
    countries: {
      australia: "Australia",
      unitedStates: "United States", 
      canada: "Canada",
      unitedKingdom: "United Kingdom",
      germany: "Germany",
      france: "France"
    },

    // Info Section
    info: {
      processVisaTitle: "Process your visa application quickly and easily",
      processVisaDesc: "We streamline the entire visa application process, making it simple and stress-free. From document verification to submission, we handle everything so you can focus on planning your trip.",
      expertSupportTitle: "Expert support every step of the way",
      expertSupportDesc: "Our experienced team provides personalized guidance throughout your application journey. Get answers to your questions and ensure your application meets all requirements.",
      secureProcessTitle: "Secure and reliable processing",
      secureProcessDesc: "Your personal information and documents are protected with bank-level security. We ensure your application is processed safely and efficiently."
    },

    // How It Works Section
    howItWorks: {
      title: "How It Works",
      step1Title: "Submit Your Application",
      step1Desc: "Fill out our simple online form with your personal details and travel information.",
      step2Title: "Upload Documents", 
      step2Desc: "Securely upload all required documents through our encrypted platform.",
      step3Title: "Track Progress",
      step3Desc: "Monitor your application status in real-time through your personal dashboard.",
      step4Title: "Receive Your Visa",
      step4Desc: "Get your approved visa delivered directly to your email or physical address."
    },

    // Testimonials Section
    testimonials: {
      title: "What Our Customers Say",
      testimonial1: "The visa application process was incredibly smooth and fast. I received my visa in just 3 days!",
      testimonial2: "Outstanding customer service and support throughout the entire process. Highly recommended!",
      testimonial3: "Professional, reliable, and efficient. Made my travel planning so much easier."
    },

    // FAQ Section
    faq: {
      title: "Frequently Asked Questions",
      question1: "How long does the visa processing take?",
      answer1: "Most visa applications are processed within 3-5 business days. Express processing is available for urgent applications.",
      question2: "What documents do I need to submit?",
      answer2: "You'll need a valid passport, recent photographs, completed application form, and supporting documents based on your visa type.",
      question3: "Is my personal information secure?",
      answer3: "Yes, we use bank-level encryption and security measures to protect all your personal information and documents.",
      question4: "Can I track my application status?",
      answer4: "Absolutely! You can track your application status in real-time through your personal dashboard.",
      contactUs: "Contact Us",
      blog: "Blog"
    },

    // Footer
    footer: {
      contactUs: "Contact Us",
      blog: "Blog"
    },

    // About Us Page
    about: {
      title: "About MEXVISA",
      subtitle: "Your trusted partner for Mexican visa applications",
      missionTitle: "Our Mission",
      missionText: "We are dedicated to simplifying the Mexican visa application process for travelers worldwide. Our mission is to provide fast, secure, and reliable visa processing services with exceptional customer support.",
      startApplication: "Start Your Application"
    },

    // Dashboard
    dashboard: {
      welcome: "Welcome to your Dashboard",
      manageApplications: "Manage your visa applications and track their progress here."
    },

    // Authentication
    auth: {
      welcomeBack: "Welcome back",
      loginSubtitle: "Please sign in to your account",
      email: "Email",
      password: "Password",
      rememberMe: "Remember me",
      forgotPassword: "Forgot your password?",
      loginButton: "Sign In",
      noAccount: "Don't have an account?",
      signUpHere: "Sign up here",
      
      // Sign Up
      createAccount: "Create your account",
      signUpSubtitle: "Join us to start your visa application",
      firstName: "First Name",
      lastName: "Last Name",
      confirmPassword: "Confirm Password",
      signUpButton: "Sign Up",
      haveAccount: "Already have an account?",
      signInHere: "Sign in here",
      
      // Common
      orContinueWith: "Or continue with",
      processing: "Processing...",
      pleaseWait: "Please wait..."
    },

    // Common
    common: {
      loading: "Loading...",
      error: "Error",
      success: "Success",
      submit: "Submit",
      cancel: "Cancel",
      continue: "Continue",
      back: "Back",
      next: "Next",
      save: "Save",
      edit: "Edit",
      delete: "Delete",
      confirm: "Confirm"
    }
  },

  ES: {
    // Navigation
    nav: {
      aboutUs: "Acerca de Nosotros",
      travelRequirements: "Requisitos de viaje",
      myApplications: "Mis solicitudes",
      signIn: "Iniciar Sesión",
      signUp: "Registrarse",
      profile: "Perfil",
      history: "Historial",
      signOut: "Cerrar Sesión",
      menu: "Menú",
      close: "Cerrar"
    },

    // Language
    language: {
      english: "English",
      spanish: "Español"
    },

    // Hero Section
    hero: {
      title: "Procesamiento de Visa Rápido, Simple y Seguro con Soporte en Vivo.",
      subtitle: "¡Tu Solución Integral para Solicitudes de Visa de Pasaporte Mexicano!",
      country: "País",
      startApplication: "Iniciar tu Solicitud"
    },

    // Countries
    countries: {
      australia: "Australia",
      unitedStates: "Estados Unidos",
      canada: "Canadá", 
      unitedKingdom: "Reino Unido",
      germany: "Alemania",
      france: "Francia"
    },

    // Info Section
    info: {
      processVisaTitle: "Procesa tu solicitud de visa rápida y fácilmente",
      processVisaDesc: "Simplificamos todo el proceso de solicitud de visa, haciéndolo simple y sin estrés. Desde la verificación de documentos hasta la presentación, manejamos todo para que puedas enfocarte en planificar tu viaje.",
      expertSupportTitle: "Soporte experto en cada paso del camino",
      expertSupportDesc: "Nuestro equipo experimentado proporciona orientación personalizada durante todo tu proceso de solicitud. Obtén respuestas a tus preguntas y asegúrate de que tu solicitud cumpla con todos los requisitos.",
      secureProcessTitle: "Procesamiento seguro y confiable",
      secureProcessDesc: "Tu información personal y documentos están protegidos con seguridad de nivel bancario. Garantizamos que tu solicitud se procese de manera segura y eficiente."
    },

    // How It Works Section
    howItWorks: {
      title: "Cómo Funciona",
      step1Title: "Envía tu Solicitud",
      step1Desc: "Completa nuestro formulario en línea simple con tus datos personales e información de viaje.",
      step2Title: "Sube Documentos",
      step2Desc: "Sube de forma segura todos los documentos requeridos a través de nuestra plataforma encriptada.",
      step3Title: "Rastrea el Progreso",
      step3Desc: "Monitorea el estado de tu solicitud en tiempo real a través de tu panel personal.",
      step4Title: "Recibe tu Visa",
      step4Desc: "Recibe tu visa aprobada directamente en tu correo electrónico o dirección física."
    },

    // Testimonials Section
    testimonials: {
      title: "Lo que Dicen Nuestros Clientes",
      testimonial1: "¡El proceso de solicitud de visa fue increíblemente fluido y rápido. Recibí mi visa en solo 3 días!",
      testimonial2: "¡Excelente servicio al cliente y soporte durante todo el proceso. Altamente recomendado!",
      testimonial3: "Profesional, confiable y eficiente. Hizo que la planificación de mi viaje fuera mucho más fácil."
    },

    // FAQ Section
    faq: {
      title: "Preguntas Frecuentes",
      question1: "¿Cuánto tiempo toma el procesamiento de la visa?",
      answer1: "La mayoría de las solicitudes de visa se procesan dentro de 3-5 días hábiles. El procesamiento express está disponible para solicitudes urgentes.",
      question2: "¿Qué documentos necesito enviar?",
      answer2: "Necesitarás un pasaporte válido, fotografías recientes, formulario de solicitud completado y documentos de apoyo basados en tu tipo de visa.",
      question3: "¿Es segura mi información personal?",
      answer3: "Sí, utilizamos encriptación y medidas de seguridad de nivel bancario para proteger toda tu información personal y documentos.",
      question4: "¿Puedo rastrear el estado de mi solicitud?",
      answer4: "¡Absolutamente! Puedes rastrear el estado de tu solicitud en tiempo real a través de tu panel personal.",
      contactUs: "Contáctanos",
      blog: "Blog"
    },

    // Footer
    footer: {
      contactUs: "Contáctanos",
      blog: "Blog"
    },

    // About Us Page
    about: {
      title: "Acerca de MEXVISA",
      subtitle: "Tu socio de confianza para solicitudes de visa mexicana",
      missionTitle: "Nuestra Misión",
      missionText: "Nos dedicamos a simplificar el proceso de solicitud de visa mexicana para viajeros de todo el mundo. Nuestra misión es proporcionar servicios de procesamiento de visa rápidos, seguros y confiables con soporte al cliente excepcional.",
      startApplication: "Iniciar tu Solicitud"
    },

    // Dashboard
    dashboard: {
      welcome: "Bienvenido a tu Panel de Control",
      manageApplications: "Gestiona tus solicitudes de visa y rastrea su progreso aquí."
    },

    // Authentication
    auth: {
      welcomeBack: "Bienvenido de vuelta",
      loginSubtitle: "Por favor inicia sesión en tu cuenta",
      email: "Correo Electrónico",
      password: "Contraseña",
      rememberMe: "Recordarme",
      forgotPassword: "¿Olvidaste tu contraseña?",
      loginButton: "Iniciar Sesión",
      noAccount: "¿No tienes una cuenta?",
      signUpHere: "Regístrate aquí",
      
      // Sign Up
      createAccount: "Crea tu cuenta",
      signUpSubtitle: "Únete a nosotros para comenzar tu solicitud de visa",
      firstName: "Nombre",
      lastName: "Apellido",
      confirmPassword: "Confirmar Contraseña",
      signUpButton: "Registrarse",
      haveAccount: "¿Ya tienes una cuenta?",
      signInHere: "Inicia sesión aquí",
      
      // Common
      orContinueWith: "O continúa con",
      processing: "Procesando...",
      pleaseWait: "Por favor espera..."
    },

    // Common
    common: {
      loading: "Cargando...",
      error: "Error",
      success: "Éxito",
      submit: "Enviar",
      cancel: "Cancelar",
      continue: "Continuar",
      back: "Atrás",
      next: "Siguiente",
      save: "Guardar",
      edit: "Editar",
      delete: "Eliminar",
      confirm: "Confirmar"
    }
  }
}; 